...
export default function QuotationHomeScreen() {
  const navigation = useNavigation<NavigationProp>();
  const [loading, setLoading] = useState(true);
  const [refreshing, setRefreshing] = useState(false);
  const [quotations, setQuotations] = useState<Quotation[]>([]);
  const [statistics, setStatistics] = useState<QuotationStatistics | null>(
    null,
  );
  ...
  const loadQuotations = async () => {
    try {
      setLoading(true);
      const response = await quotationService.list(filters);
      ...
      Alert.alert("Error", error.message || "Failed to load quotations");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleFilterChange = (key: keyof QuotationListParams, value: any) => {
          ...
          <View style={styles.statsSection}>
            ...
            <View style={styles.statsGrid}>
              ...
              <LinearGradient
                ...
                style={styles.statCard}>
                <Text style={styles.statValue}>{statistics.accepted}</Text>
          ...
          {quotations.length === 0 ? (
            ...
            quotations.map((quotation) => (
              <TouchableOpacity
                ...
                style={styles.card}
                onPress={() =>
                  navigation.navigate("QuotationShow", { id: quotation.id })
                }>

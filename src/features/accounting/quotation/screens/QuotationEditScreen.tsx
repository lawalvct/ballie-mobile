...
export default function QuotationEditScreen() {
  ...
  const loadQuotation = async () => {
    try {
      setLoading(true);
      const quotation = await quotationService.show(id);

      if (quotation.status !== "draft") {
        Alert.alert("Cannot Edit", "Only draft quotations can be edited.", [
      ...
      setCustomerLedgerId(
        quotation.customer_ledger_id
          ? String(quotation.customer_ledger_id)
          : "",
      );
      setCustomerSearch(
      ...
      setTerms(quotation.terms_and_conditions || "");
      setNotes(quotation.notes || "");

      const productSearchMap: Record<string, string> = {};
      const mappedItems = (quotation.items || []).map((item) => {
        const localId = String(
          item.id ?? `temp-${Date.now()}-${Math.random()}`,
        );
        productSearchMap[localId] = item.product_name || "";
        return {
          local_id: localId,
      ...
      setProductSearches(productSearchMap);
    } catch (error: any) {
  ...
  const handleSubmit = async (action: "save" | "save_and_send") => {
    ...
    try {
      ...
      Alert.alert("Error", error.message || "Failed to update quotation");
    } finally {
      setSubmitting(false);
    }
  };
        ...
        <View style={styles.formSection}>
          ...
          <View style={styles.formGroup}>
            ...
            {showQuotationDatePicker && (
              <DateTimePicker
                value={quotationDate ? new Date(quotationDate) : new Date()}
                mode="date"
                display="default"
        ...
        <View style={styles.actionsSection}>
          <TouchableOpacity
            ...
            {submitting ? (
              ...
              <Text style={styles.submitBtnText}>Update Draft</Text>
            )}
          </TouchableOpacity>
          <TouchableOpacity
            ...
            {submitting ? (
              ...
              <Text style={styles.submitBtnText}>Update & Send</Text>
            )}
          </TouchableOpacity>
        </View>
      </ScrollView>

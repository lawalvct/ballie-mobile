...
export default function LedgerAccountEditScreen({ navigation, route }: Props) {
  ...
  const loadData = async () => {
    try {
      ...
      // Load form data and account details in parallel
      const [formDataResponse, accountData] = await Promise.all([
        ledgerAccountService.getFormData(),
        ledgerAccountService.show(id),
      ]);

      setFormData(formDataResponse);
      setSelectedType(accountData.account_type);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await ledgerAccountService.update(id, {
      ...
      showToast("✅ Ledger account updated successfully", "success");
      ...
      if (error.errors) {
        ...
        showToast(error.message || "Failed to update ledger account", "error");
      }
    } finally {
      setSubmitting(false);
    }
  ...
  // Filter parent accounts by selected type (exclude self)
  const availableParents =
    formData?.parent_accounts.filter(
      (parent) => parent.account_type === selectedType && parent.id !== id
    ) || [];
    ...
    return (
      <SafeAreaView style={styles.container}>
        ...
        <View style={styles.header}>
          ...
          <Text style={styles.title}>Edit Ledger Account</Text>
        </View>

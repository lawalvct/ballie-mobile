...
import type {
  ...
  PayrollSalaryComponentListResponse,
} from "../types";

class SalaryComponentService {
  private baseUrl = "/payroll/salary-components";

  async list(
    params: PayrollSalaryComponentListParams = {},
  ): Promise<PayrollSalaryComponentListResponse> {
    ...
    const total =
      typeof totalFromSource === "number" ? totalFromSource : components.length;
    const per_page =
    ...
    const to =
      typeof paginationSource?.to === "number"
        ...
        : total === 0
          ? 0
          : Math.min(total, from + components.length - 1);
  ...
  async show(id: number): Promise<PayrollSalaryComponent> {
    const response = await apiClient.get(`${this.baseUrl}/${id}`);
    const payload = (response as any)?.data ?? response;
    return payload?.data?.component || payload?.component || payload?.data;
  }

  async create(
    data: Partial<PayrollSalaryComponent>,

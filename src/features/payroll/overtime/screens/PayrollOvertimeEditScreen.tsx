...
export default function PayrollOvertimeEditScreen({
  navigation,
  route,
}: Props) {
  ...
  const loadOvertime = async () => {
    try {
      ...
      setEmployeeId(overtime.employee_id);
      setOvertimeDate(overtime.overtime_date || formatDate(new Date()));
      setCalculationMethod(overtime.calculation_method || "hourly");
      setStartTime(overtime.start_time || "");
      setEndTime(overtime.end_time || "");
      setOvertimeType(overtime.overtime_type || "weekday");
      setHourlyRate(overtime.hourly_rate ? String(overtime.hourly_rate) : "");
      setFixedAmount(
        overtime.total_amount ? String(overtime.total_amount) : "",
      );
      setReason(overtime.reason || "");
      setWorkDescription(overtime.work_description || "");
      setStatus(overtime.status);
    } catch (error: any) {
  ...
  const handleSubmit = async () => {
    if (status && status !== "pending") {
      showToast("Only pending overtime can be edited", "error");
      return;
    }
    if (!employeeId) {
    ...
    if (calculationMethod === "hourly") {
      ...
      try {
        ...
        showToast(error.message || "Failed to update overtime", "error");
      } finally {
        setSubmitting(false);
      }
      return;
    ...
    return (
      <SafeAreaView style={styles.container}>
        ...
        <View style={styles.header}>
          ...
          <Text style={styles.headerTitle}>Edit Overtime</Text>
        ...
        <View style={styles.formSection}>
          ...
          <TouchableOpacity
            ...
            <Text style={styles.submitButtonText}>
              {submitting ? "Saving..." : "Update Overtime"}
            </Text>
          </TouchableOpacity>
        </View>

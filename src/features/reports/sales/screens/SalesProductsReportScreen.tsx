...
import type { SalesProductRecord } from "../../types";
...
export default function SalesProductsReportScreen({ navigation }: Props) {
  ...
  const [records, setRecords] = useState<SalesProductRecord[]>([]);
  ...
  const loadData = async () => {
    try {
      setLoading(true);
      const response = await reportsService.salesProducts({
        from_date: fromDate,
        to_date: toDate,
        sort_by: sortBy,
      ...
      showToast(error.message || "Failed to load product sales", "error");
    } finally {
      setLoading(false);
    }
  };
            ...
            records.map((item, index) => (
              <View
                ...
                <Text style={styles.recordSubtext}>
                  Qty: {item.total_quantity ?? 0} • Revenue:{" "}
                  {formatAmount(item.total_revenue)}
                </Text>
                <Text style={styles.recordSubtext}>
                  Cost: {formatAmount(item.total_cost)} • Profit:{" "}
                  {formatAmount(item.total_profit)}
                </Text>
              </View>
            ))
          ) : (

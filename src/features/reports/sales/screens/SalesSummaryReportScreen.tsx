...
import type { ReportGroupBy, SalesSummaryResponse } from "../../types";
...
export default function SalesSummaryReportScreen({ navigation }: Props) {
  ...
  const loadData = async () => {
    try {
      setLoading(true);
      const response = await reportsService.salesSummary({
        from_date: fromDate,
        to_date: toDate,
        group_by: groupBy,
      });
      ...
      showToast(error.message || "Failed to load sales summary", "error");
    } finally {
      setLoading(false);
    }
  };
  ...
  const summaryCards = useMemo(
    () => [
      {
        label: "Total Sales",
        value: formatAmount(data?.summary?.total_sales),
        colors: ["#8B5CF6", "#6D28D9"] as const,
      },
      {
        label: "Sales Count",
        value: String(data?.summary?.sales_count ?? 0),
        colors: ["#10B981", "#059669"] as const,
      },
      {
        label: "Average Sale",
        value: formatAmount(data?.summary?.average_sale_value),
      ...
      <ScrollView style={styles.content}>
        ...
        {loading ? (
          <View style={styles.loadingContainer}>
            ...
            <Text style={styles.loadingText}>Loading sales summary...</Text>
          </View>
        ) : (
          <>

...
import type { PayrollBankScheduleReportResponse } from "../../types";
import { showToast } from "../../../../utils/toast";

type Props = NativeStackScreenProps<
  ReportsStackParamList,
  "PayrollBankScheduleReport"
>;
...
export default function PayrollBankScheduleReportScreen({ navigation }: Props) {
  const currentYear = new Date().getFullYear();
  const [year, setYear] = useState(currentYear);
  const [month, setMonth] = useState<number | undefined>();
  const [status, setStatus] = useState<string | undefined>();
  const [loading, setLoading] = useState(true);
  const [data, setData] = useState<PayrollBankScheduleReportResponse | null>(
    null,
  );
  ...
  const loadData = async () => {
    try {
      setLoading(true);
      const response = await reportsService.payrollBankSchedule({
        year,
        month,
        status,
      });
      ...
      showToast(error.message || "Failed to load bank schedule", "error");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Bank Schedule</Text>
      ...
      <ScrollView style={styles.content}>
        ...
        {loading ? (
          <View style={styles.loadingContainer}>
            ...
            <Text style={styles.loadingText}>Loading bank schedule...</Text>
          </View>
        ) : (
          <>
            ...
            <View style={styles.listSection}>
              ...
              {data?.periods?.length ? (
                ...
                <Text style={styles.emptyText}>No payroll periods.</Text>
              )}
            </View>
          </>
        )}

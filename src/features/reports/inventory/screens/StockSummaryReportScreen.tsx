...
export default function StockSummaryReportScreen({ navigation }: Props) {
  ...
  const [data, setData] = useState<StockSummaryReportResponse | null>(null);

  useEffect(() => {
    loadData();
  }, [asOfDate, categoryId, stockStatus, sortBy, sortOrder, searchQuery]);

  const loadData = async () => {
    try {
      setLoading(true);
      const response = await reportsService.inventoryStockSummary({
        as_of_date: asOfDate,
      ...
      showToast(error.message || "Failed to load stock summary", "error");
    } finally {
      setLoading(false);
    }
  };
  ...
  const summaryCards = useMemo(
    () => [
      ...
      },
      {
        ...
        value: String(data?.summary?.total_stock_quantity ?? 0),
        colors: ["#10B981", "#059669"] as const,
      },
      {
        label: "Low Stock",
        value: String(data?.summary?.low_stock_count ?? 0),
      ...
      <ScrollView style={styles.content}>
        ...
        {loading ? (
          <View style={styles.loadingContainer}>
            ...
            <Text style={styles.loadingText}>Loading stock summary...</Text>
          </View>
        ) : (
          <>
                ...
                records.map((item, index) => (
                  <View style={styles.recordCard} key={`${item.id}-${index}`}>
                    ...
                    <Text style={styles.recordSubtext}>
                      ...
                      {item.primary_unit?.abbreviation || ""}
                    </Text>
                    <Text style={styles.recordSubtext}>
                      Stock: {item.calculated_stock ?? 0} • Rate:{" "}
                      {formatAmount(item.average_rate)}
                    </Text>
                    <Text style={styles.recordSubtext}>
                      Value: {formatAmount(item.calculated_value)} • Status:{" "}
                      {item.status_flag || "-"}
                    </Text>
                  </View>
                ))

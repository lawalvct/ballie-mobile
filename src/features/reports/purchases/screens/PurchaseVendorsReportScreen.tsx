...
import type { PurchaseVendorRecord } from "../../types";
import { showToast } from "../../../../utils/toast";

type Props = NativeStackScreenProps<
  ReportsStackParamList,
  "PurchaseVendorsReport"
>;
...
export default function PurchaseVendorsReportScreen({ navigation }: Props) {
  ...
  const [records, setRecords] = useState<PurchaseVendorRecord[]>([]);
  ...
  const loadData = async () => {
    try {
      setLoading(true);
      const response = await reportsService.purchaseVendors({
        from_date: fromDate,
        to_date: toDate,
        sort_by: sortBy,
      ...
      showToast(error.message || "Failed to load vendor purchases", "error");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Vendor Purchases</Text>
          ...
          {loading ? (
            ...
            records.map((item, index) => (
              <View style={styles.recordCard} key={`${item.id}-${index}`}>
                <Text style={styles.recordTitle}>{item.name || "Vendor"}</Text>
            ...
            <Text style={styles.emptyText}>No vendor purchases found.</Text>
          )}
        </View>

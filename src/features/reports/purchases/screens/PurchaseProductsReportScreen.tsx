...
import type { PurchaseProductRecord } from "../../types";
...
export default function PurchaseProductsReportScreen({ navigation }: Props) {
  ...
  const [records, setRecords] = useState<PurchaseProductRecord[]>([]);
  ...
  const loadData = async () => {
    try {
      setLoading(true);
      const response = await reportsService.purchaseProducts({
        from_date: fromDate,
        to_date: toDate,
        sort_by: sortBy,
      ...
      showToast(error.message || "Failed to load product purchases", "error");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Product Purchases</Text>
            ...
            records.map((item, index) => (
              <View
                ...
                <Text style={styles.recordSubtext}>
                  Qty: {item.total_quantity ?? 0} • Cost:{" "}
                  {formatAmount(item.total_cost)}
                </Text>
              </View>
            ))
          ) : (
            <Text style={styles.emptyText}>No product purchases found.</Text>
          )}
        </View>

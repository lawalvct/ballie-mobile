...
type Props = NativeStackScreenProps<InventoryStackParamList, "UnitCreate">;
...
export default function UnitCreateScreen({ navigation, route }: Props) {
  ...
  const loadFormData = async () => {
    try {
      setLoading(true);
      const response = await unitService.getFormData();
      setBaseUnits(response.base_units || []);
    } catch (error: any) {
  ...
  const handleSubmit = async () => {
    ...
    if (!isBaseUnit) {
      ...
      if (Number(conversionFactor) <= 0) {
        showToast("Conversion factor must be greater than 0", "error");
        return;
      }
    }
    ...
    try {
      ...
      showToast(error.message || "Failed to create unit", "error");
    } finally {
      setSubmitting(false);
    }
  };
            ...
            <View style={styles.switchRow}>
              <View style={styles.switchLabel}>
                ...
                <Text style={styles.helperText}>
                  {isBaseUnit
                    ? "This is a base unit (e.g., Kilogram, Meter, Liter)"
                    : "This is a derived unit (e.g., Gram, Centimeter, Milliliter)"}
                </Text>
              </View>
              <Switch
                value={isBaseUnit}
          ...
          {!isBaseUnit && (
            <>
              <View style={styles.formGroup}>
                ...
                <Text style={styles.helperText}>
                  Choose the base unit this will be converted from
                </Text>
              </View>
              ...
              <View style={styles.formGroup}>
                ...
                />
                <Text style={styles.helperText}>
                  How many base units equal 1 of this unit. Example: 1 gram =
                  0.001 kilogram
                </Text>
              </View>
            </>
        ...
        <View style={styles.buttonContainer}>
          <TouchableOpacity
            ...
            {submitting ? (
              ...
              <Text style={styles.submitButtonText}>Create Unit</Text>
            )}
          </TouchableOpacity>
        </View>

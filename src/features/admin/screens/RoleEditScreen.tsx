...
import type { UpdateRolePayload, ModulePermissions } from "../types";

type Nav = NativeStackNavigationProp<AdminStackParamList, "RoleEdit">;
type Route = RouteProp<AdminStackParamList, "RoleEdit">;
...
export default function RoleEditScreen() {
  ...
  const { role, isLoading: loadingRole } = useRoleDetail(id);
  const { permissionsByModule, isLoading: loadingPerms } = useRoleFormData();
  const updateRole = useUpdateRole(id);

  const [form, setForm] = useState<UpdateRolePayload>({
    name: "",
    description: "",
    permissions: [],
    is_active: true,
  ...
  if (loadingRole || loadingPerms || !role) {
    return (
      ...
      <ScrollView
        ...
        <View style={styles.formCard}>
          ...
          <Text style={styles.sectionSubtitle}>
            {form.permissions?.length || 0} permissions selected
          </Text>
          ...
          {permissionsByModule.map((module: ModulePermissions) => {
            ...
            const allSelected =
              permIds.length > 0 && selectedCount === permIds.length;
        ...
        <TouchableOpacity
          style={[
            styles.submitBtn,
            updateRole.isPending && styles.submitBtnDisabled,
          ]}
